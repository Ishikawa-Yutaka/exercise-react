/**
 * React Form Components - InputField Subtests inside ContactForm.test.tsx
 * InputFieldコンポーネントのテスト（ContactForm.test.tsx 内のセクション）
 */
...
describe("InputField", () => {
  ...
  describe("基本レンダリング", () => {
    ...
    it("異なる入力タイプが設定される", () => {
      const { rerender } = render(
        <InputField {...defaultProps} type="email" />
      );
  ...
  describe("エラー状態", () => {
    ...
    it("touchedがtrueでエラーがある場合にエラーメッセージが表示される", () => {
      render(<InputField {...defaultProps} error="エラーメッセージ" touched />);
    ...
    it("エラー状態でaria属性が正しく設定される（idは field-${name}-error）", () => {
      render(<InputField {...defaultProps} error="エラーメッセージ" touched />);

      const input = screen.getByRole("textbox");
      expect(input).toHaveAttribute("aria-invalid", "true");
      // 実装は `${fieldId}-error` で、fieldId は `field-${name}`
  ...
  describe("ユーザーインタラクション", () => {
    it("入力時にonChangeが呼ばれる", async () => {
      ...
      // 'テ','ス','ト' の3回（IME シミュレーション環境によっては差が出ることがあります）
  ...
  // ─────────────────────────────────────────────
  describe("アクセシビリティ", () => {
    it("ラベルと入力フィールドが正しく関連付けられている（id/for は field-${name} を使用）", () => {
      render(<InputField {...defaultProps} />);
  ...
  describe("フォーカス管理", () => {
    ...
    it("フィールドIDが一意になる（id は field-${name} を使用）", async () => {
      const user = userEvent.setup();
      ...
      expect(screen.getByLabelText("フィールド1")).toHaveAttribute(
        "id",
        "field-field1"
      );
      expect(screen.getByLabelText("フィールド2")).toHaveAttribute(
        "id",
        "field-field2"
      );
